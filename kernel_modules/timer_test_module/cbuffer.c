  ...
  if ( cbuffer->size == cbuffer->max_size )
    {
      ...
      cbuffer->data[cbuffer->head*sizeof(int)]=new_item;
	...
	pos= ( cbuffer->head+cbuffer->size ) % cbuffer->max_size;
      cbuffer->data[pos*sizeof(int)]=new_item;
      cbuffer->size++;
    }
}
...
void insert_items_cbuffer_t ( cbuffer_t* cbuffer, const unsigned int* items, int nr_items)
{
  ...
  if (whead+nr_items_left > cbuffer->max_size)
    {
      ...
      memcpy(&cbuffer->data[whead*sizeof(int)],items,items_copied);
  ...
  if (nr_items_left)
    {
      memcpy(&cbuffer->data[whead*sizeof(int)],items,nr_items_left);
      whead+=nr_items_left;
    }
  ...
  if (nr_items_left)
    {
      ...
      cbuffer->head+=(nr_items_left*sizeof(int));
    }
...
unsigned int remove_cbuffer_t ( cbuffer_t* cbuffer)
{
  ...
  if ( cbuffer->size !=0 )
    {
      ret=cbuffer->data[cbuffer->head*sizeof(int)];	
      cbuffer->head= ( cbuffer->head+1 ) % cbuffer->max_size;
      cbuffer->size--;
    }
...
{
  if ( cbuffer->size !=0 )
    return &cbuffer->data[cbuffer->head*sizeof(int)];
  else{
    return NULL;
  }
}
