	...
	public void step() throws InstructionException{
		...
		if (_program.getInstructionAt(_uc.getCP())!=null){
			...
			try {
				...
				for(Observer o: _observers)
					o.onError(e);
				//Si hay error, escribes los datos de la cpu en un archivo.
				try {
					PrintStream coreDumpLog = new PrintStream("coreDump.log");
					coreDumpLog.println("Error en la instruccion: " + _program.getInstructionAt(_uc.getCP()));
					coreDumpLog.println("cp: " + _uc.getData().getCp());
					coreDumpLog.print(toString());
					coreDumpLog.close();
				} catch (FileNotFoundException e1) {}
				//Fin de la escritura del log file.
