...
public class CPU extends Observable<CPU.Observer>{
	...
	private ArrayList<mv.model.ControlUnit.Data> _undoRecordUC;
	private ArrayList<mv.model.OperandStack.Data> _undoRecordStack;
	private ArrayList<mv.model.Memory.Data> _undoRecordMemory;
	...
	public CPU(){
		...
		_undoRecordUC = new ArrayList<mv.model.ControlUnit.Data>();
		_undoRecordStack = new ArrayList<mv.model.OperandStack.Data>();
		_undoRecordMemory = new ArrayList<mv.model.Memory.Data>();
		_undoRecordUC.add(_uc.getData());
		_undoRecordStack.add(_pila.getData());
		_undoRecordMemory.add(_memoria.getData());
	}
	...
	public void undo(){
		if(_undoRecordUC.size()<2 && _undoRecordStack.size()<2 &&_undoRecordMemory.size()<2)
			return;
		_undoRecordUC.remove(_undoRecordUC.size()-1);
		_undoRecordStack.remove(_undoRecordStack.size()-1);
		_undoRecordMemory.remove(_undoRecordMemory.size()-1);
		_uc.setData(_undoRecordUC.get(_undoRecordUC.size()-1));
		_pila.setData(_undoRecordStack.get(_undoRecordStack.size()-1));
		_memoria.setData(_undoRecordMemory.get(_undoRecordMemory.size()-1));
	}
}
